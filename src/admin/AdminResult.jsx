...
function AdminResult() {
  ...
  const fetchResults = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/candidate-results/all')
      if (response.ok) {
        const data = await response.json()
        // Sort results in reverse order (most recent first)
        const sortedResults = (data.results || []).sort((a, b) => {
          // Try to sort by testDate, then by id, then by any timestamp field
          ...
          // Fallback: reverse the array order
          return -1
        })
  ...
  const handleDeleteResult = async (result) => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/candidate-results/${resultId}`, {
        method: 'DELETE'
      })
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <div className="results-pagination-controls d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                      ...
                      <nav aria-label="Aptitude result pagination">
